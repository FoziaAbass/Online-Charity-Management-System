...
app.put('/update-details', (request, response) => {
    ...
    const sql = `UPDATE donors SET First_Name = '${Fname}', Last_Name = '${Lname}', Phone = '${Phone}', Email = '${Encrypt(Email)}' where Donor_ID = '${id}'`
    const sql2 = `UPDATE organizations SET Name = '${Name}', Email = '${Encrypt(Email)}', Description = '${Description}', Address = '${Address}' where Organization_ID = '${id}'`;
    connection.query(table === 'donors' ? sql : sql2, (error, result) => {
        if (error) throw error;
        // console.log(result)
        console.log("Updated")
...
app.post('/signup-donor', (request, response) => {
    ...
    Donor_info.Donor_Email = Encrypt(Donor_info.Donor_Email)
    Donor_info.Donor_Password = Encrypt(Donor_info.Donor_Password)
    try {
...
app.post('/signup-organization', (request, response) => {
    ...
    Organization_info.Organization_Email = Encrypt(Organization_info.Organization_Email)
    Organization_info.Organization_Password = Encrypt(Organization_info.Organization_Password)
    try {
...
app.post('/login', async(request, response) => {
    ...
    if (type_of_user === 'Donor') {
        connection.query('SELECT * FROM donors WHERE Email = ? AND Password = ?', [Encrypt(email), Encrypt(password)], (error, results, fields) => {
            if (error) throw error;
            if (results.length) {

                session = request.session
                session.user = JSON.parse(JSON.stringify(results[0]))
                session.user.Email = Decrypt(session.user.Email)
                console.log(session);
                session.save()
        ...
        connection.query('SELECT * FROM organizations WHERE Email = ? AND Password = ?', [Encrypt(email), Encrypt(password)], (error, results, fields) => {
            if (error) throw error;
            if (results.length) {
                session = request.session
                session.user = JSON.parse(JSON.stringify(results[0]))
